...
import type { GroupDisplay, MatchDisplay } from '../types';
...
export default function GroupCard({
  ...
  const sortedMatches = [...group.matches].sort(
    (a, b) => a.match.matchNumber - b.match.matchNumber,
  );

  const headerColor =
    GROUP_COLORS[group.letter] || 'from-slate-600 to-slate-700';
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      >
        ...
        <span
          className="text-xs opacity-80 transition-transform duration-200"
        ...
        <table className="w-full text-xs">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-slate-500 uppercase text-[10px]">
              ...
              <th className="px-1 py-1.5 text-center w-7" title="נקודות">
                נק׳
              </th>
              <th className="px-1 py-1.5 text-center w-7" title="משחקים">
                מש׳
              </th>
              <th className="px-1 py-1.5 text-center w-8" title="הפרש שערים">
                ±
              </th>
            </tr>
          </thead>
            ...
            {group.standings.map((standing, idx) => (
              ...
              >
                ...
                <td className="px-1.5 py-1.5 text-right">
                  <span
                    className="font-medium text-slate-800 truncate block max-w-[80px]"
                    title={standing.team?.name}
                  >
                    {standing.team?.nameHebrew || standing.team?.name || 'TBD'}
                  </span>
                </td>
                <td className="px-1 py-1.5 text-center font-bold text-slate-900">
                  {standing.points}
                </td>
      ...
      {expanded && sortedMatches.length > 0 && (
        <div className="border-t border-slate-200 bg-slate-50/50 p-2 space-y-1.5">
          <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide px-1">
            משחקי בית
          </div>
...
function MatchRow({
  ...
  const team1Name =
    team1?.nameHebrew || team1?.name || match.team1Name || match.team1Code;
  const team2Name =
    team2?.nameHebrew || team2?.name || match.team2Name || match.team2Code;
