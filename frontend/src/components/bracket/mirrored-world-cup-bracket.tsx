...
import type { Team, Match, Round, BracketState, MirroredBracketProps, RoundConfig } from "./types"
...
const ROUND_CONFIGS: Record<Round, RoundConfig> = {
  ...
  quarterfinals: { name: "quarterfinals", matches: 2, displayName: "Quarterfinals" },
...
export default function MirroredWorldCupBracket({
  ...
  const advanceToThirdPlace = (state: BracketState, side: "left" | "right", loser: Team) => {
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center gap-3">
              <div className="flex flex-col items-center gap-1">
                <Label className="text-xs text-blue-700">{match.team1?.name || "TBD"}</Label>
                <Input
                  type="number"
                  min="0"
                  value={match.score1}
              ...
              <div className="flex flex-col items-center gap-1">
                <Label className="text-xs text-blue-700">{match.team2?.name || "TBD"}</Label>
                <Input
                  type="number"
                  min="0"
                  value={match.score2}
            ...
            {isTied && hasTeams && (
              <div className="pt-2 border-t border-blue-200">
                <Label className="text-xs text-blue-700 mb-1 block text-center">Winner:</Label>
                <RadioGroup
  ...
  const renderSideRounds = (side: "left" | "right") => {
    const rounds: Round[] = ["round32", "round16", "quarterfinals", "semifinals"]
    ...
    return (
      <div className="flex flex-col gap-8">
        {rounds.map((round) => {
          ...
          return (
            <div key={`${side}-${round}`} className="flex flex-col items-center gap-4">
              <h4 className="text-sm font-bold text-blue-900 text-center">{ROUND_CONFIGS[round].displayName}</h4>
              <div className="flex flex-col gap-3">{roundMatches.map((match) => renderMatch(match, true))}</div>
            </div>
          )
        })}
      </div>
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {bracketState.thirdPlace && (
            <Badge className="bg-orange-500 text-white px-3 py-1">3rd Place: {bracketState.thirdPlace.name}</Badge>
          )}
        </div>
      </div>
    )
  ...
  return (
    <div className={`w-full bg-gradient-to-br from-blue-100 to-green-100 p-6 rounded-lg ${className}`}>
      ...
      <div className="grid grid-cols-3 gap-8 max-w-7xl mx-auto">
        ...
        {/* Center Column */}
        <div className="flex flex-col justify-center">{renderCenterColumn()}</div>

        {/* Right Bracket */}
        <div className="flex flex-col items-start">
          <h2 className="text-lg font-bold text-blue-900 mb-4">Right Bracket</h2>
