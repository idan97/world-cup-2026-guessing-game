...
import type { Team, Match, Round, BracketSide, BracketData, WorldCupBracketProps, RoundInfo } from "@/lib/bracket-types"

const ROUND_INFO: Record<Round, RoundInfo> = {
  round32: { name: "round32", displayName: "Round of 32", matchCount: 16, teamsPerSide: 16 },
  round16: { name: "round16", displayName: "Round of 16", matchCount: 8, teamsPerSide: 8 },
  quarterfinals: { name: "quarterfinals", displayName: "Quarterfinals", matchCount: 4, teamsPerSide: 4 },
  semifinals: { name: "semifinals", displayName: "Semifinals", matchCount: 2, teamsPerSide: 2 },
  final: { name: "final", displayName: "Final", matchCount: 1, teamsPerSide: 1 },
  thirdPlace: { name: "thirdPlace", displayName: "Third Place", matchCount: 1, teamsPerSide: 1 },
}
...
export default function WorldCupKnockoutBracket({
  ...
  const advanceWinner = (bracketData: BracketData, match: Match, winner: Team) => {
  ...
  const advanceLoserToThirdPlace = (bracketData: BracketData, side: BracketSide, loser: Team) => {
  ...
  const renderMatch = (match: Match) => {
    ...
    const isSpecialMatch = match.round === "final" || match.round === "thirdPlace"
        ...
        <CardContent className="p-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              <Label className="text-gray-700 text-sm font-medium flex-1 truncate">{match.team1?.name || ""}</Label>
            ...
            {/* Team 2 */}
            <div className="flex items-center justify-between gap-2">
              <Label className="text-gray-700 text-sm font-medium flex-1 truncate">{match.team2?.name || ""}</Label>
              <div className="flex items-center gap-2">
                <Input
                  type="number"
                  min="0"
                ...
                {isTied && hasTeams && (
                  <RadioGroup
                    value={match.winner?.id || ""}
                    onValueChange={(value) => {
                      const winner = value === match.team2?.id ? match.team2 : match.team1
                      updateMatch(match.id, { winner })
                    }}
                    className="flex"
                  >
      ...
      <div className="flex flex-col gap-8 items-center mt-32">
        <div className="flex flex-col items-center gap-2">
          ...
          {bracketData.champion && (
            <Badge className="bg-amber-500 text-white mt-2 px-2 py-1">🏆 Champion: {bracketData.champion.name}</Badge>
          )}
        </div>
  ...
  return (
    <div className={`w-full bg-white border border-gray-200 shadow-sm p-8 rounded-lg ${className}`}>
      {/* Header */}
